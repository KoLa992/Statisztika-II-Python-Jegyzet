...
Ezek után az eredmény data frame minden oszlopát, és a `groupby`-ban használt nominális változó egyedi értékeit, a pártfpreferenciákat mentsük ki külön objektumokba.

```{python}
minta_atlagok = PartPrefKimutatas.Atlag
becslesi_varok = PartPrefKimutatas.BecslesiVar
partok = PartPrefKimutatas.index
```
...
Az eredménytáblát a `summary_frame` metódussal tudjuk levadászni. Az eredményül kapott táblában az utolsó két oszloppal és utolsó négy sorral nekünk nem kell foglalkozni, így csak az első négy oszlopot és az első négy sort választjuk csak ki a megjelenítéskor. A soroknél azért csak az első négyet, mert négy egyedi pártpreferencia értékünk van csak most! Egyébként a nominális ismérvünk egyedi értékeinek számához igazítjuk mindig a kiválasztást! . A `ci_low` és `ci_upp` oszlopok rendre a konfidencia intervallumunk alsó-felső határai.

```{python}
from statsmodels.stats.meta_analysis import combine_effects

eredmeny = combine_effects(
  effect = minta_atlagok,
  variance = becslesi_varok,
  row_names = partok,
  alpha = (1-0.99)
)
...
Remek! :) Értelmezés TODO

A `statsmodels` csomag szépsége, hogy van egy `dot_plot` című függvénye, amivel csudiszép vizuális megjelenítés adható a csoportátlagoknak és a $99\%$-os megbízhatóságú konfidencia-intervallumaiknak. Ezeket az ábrákat **forest diagram**oknak hívják
