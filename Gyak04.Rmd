...
Ezek után igazából csak annyi a feladatunk, hogy a teljes becslési hibahatárt kiszámítsuk $\triangle = t_{1-\frac{\alpha}{2}}^{n-1} \times \frac{s}{\sqrt{n}}\right)$ módon, és ezt rámérjük $\pm$ a mintaátlagra, $\bar{y}$-ra. Ez már mehet ugyan úgy, mint az a) feladatban.
...
Ha valamelyik oszlopban ürrs értéket találunk, akkor az adott sorban lévő kitöltő nem válaszolt a kérdésre. Az adatbázisban szereplő kitöltők a teljes 18 év feletti magyar népességből vett véletlen mintaként kezelhetők. Most feltesszük, hogy ez a véletlen minta visszatevéses, azaz $FAE$ is. A következő tananyagban látni fogjuk, hogy ez nem is valóságtól elrugaszkodott feltevés.
...
Mehetünk simán előre a $\triangle = t_{1-\frac{\alpha}{2}}^{n-1} \times \frac{s}{\sqrt{n}}\right)$ képletet használva.
...
Láthatjuk, hogy csak a tizedesjegyekben vannak nagyon minimális eltérések. Szóval **nagy mintaelemszám esetén a megbízhatósági szorzó nyugodtan számolható standard normális eloszlésból is a t-eloszlás helyett**.
...
Ehhez az út ott kezdődik, hogy a nominális ismérvünk, azaz a pártpreferemcia egyedi értékei szerint vesszük az átlaggal vizsgált mennyiségi ismérv részátlagait, és ezen részátlagok $SH^2$-eit, azaz *becslési varianciáit*. Ezt a kiinduló data frame `groupby` metódusával gyorsan meg tudjuk oldani.
...
# SH négyzetre emelés
PartPrefKimutatas['BecslesiVar'] = PartPrefKimutatas['SH']**2
...
Majd importákjuk a csomag `combine_effects` függvényét. Ha ennek a függvénynek az `effect` paraméterében megadjuk a mért mintaátlagokat, a `variance` paraméterben az $SH^2$-ket, `row_names` paraméternél a nominális kategóriáink neveit (most a pártpreferenciák lehetséges egyedi értékeit) és az `alpha` paraméterben megadjuk az $\alpha$-t (itt tényleg a megengedett *hibavalószínűséget* hívja a gépállat `alpha`-nak, ahogy kell), akkor a függvény kiszámolja **minden nominális csoportra**, azaz esetünkben pártpreferenciára, a **vizsgált részátlagok $1-\alpha$ megbízhatóságú konfidencia-intervallumát**.<br>
Annyi szépséghibája van csak a függvénynek, hogy mindezt **nagy mintát feltételezve teszi, tehát a $k$ megbízhatósági szorzót mindig standard normális eloszlásból számolja**. Szóval kis mintáknál ($n \leq 30$) a függvény a valós konfidencia-intervallumot *alulbecsli, azaz az eredmény túl szűk lesz*!<br>
Ez most nekünk **csak az $n=13$ elemszámú 'Egyéb' pártpreferencia csoportban lehet problémás**! 

Az eredménytáblát a `summary_frame` metódussal tudjuk levadászni. Az eredményül kapott táblában az utolsó két oszloppal és utolsó négy sorral nekünk nem kell foglalkozni, így csak az első négy oszlopot és az első négy sort választjuk csak ki a megjelenítéskor. A soroknál azért csak az első négyet, mert *négy egyedi pártpreferencia értékünk van* itt és most! Egyébként a nominális ismérvünk egyedi értékeinek számához igazítjuk mindig a kiválasztást! . A `ci_low` és `ci_upp` oszlopok rendre a konfidencia intervallumunk alsó-felső határai.

```{python}
from statsmodels.stats.meta_analysis import combine_effects

eredmeny = combine_effects(
  effect = PartPrefKimutatas.Atlag,
  variance = PartPrefKimutatas.BecslesiVar,
  row_names = PartPrefKimutatas.index,
  alpha = (1-0.99)
)
eredmeny_tabla = eredmeny.summary_frame().iloc[0:4,0:4]
eredmeny_tabla
```

Remek! :) Az `eff` oszlop tartalmazza a mintaátlagokat, míg a `sd_eff` oszlop a $SH$-k lakóhelye. Tehát, ezek alapján azt mondhatjuk, hogy az egyesült ellenzék esetén egy átlagos kérdőívkitöltő $228.8$ percet netezik naponta, míg egy átlag Fidesz kitöltő csak napi $158.7$ percet.<br>
Azonban, a teljes ellenzéki népességben az átlag netezési idő $204$ és $253$ perc között van $99\%$-os valószínűséggel. A teljes fideszes táborban pedig az átlagos neten töltött idő csak $143$ és $175$ perc között mozog, szintén $99\%$-os valószínűséggel.<br>
Tehát, $99\%$-os valószínűséggel az átlag netezési idő az Ellenzék esetében magasabb, mint a Fidesz esetében a teljes népességben is, hiszen a "legrosszabb" ellenzéki átlagérték is már $204$ perc, míg a Fidesz esetén a "legjobb" átlagidő is csak $175$ perc.<br>
...
A `dot_plot` függvény `points` paraméterébe kerülnek a mintaátlagok, míg az `intervals` paraméterbe egy olyan $2$ oszlopos data frame kerül, ami tartalmazza a konfidencia-intervallum alsó és felső határának távolságát a mintaátlagoktól abszolút értékben, azaz lényegében a $\triangle$-t. Igen, itt egy olyan data frame kell neki, ahol mindkét oszlopban ugyan az az érték szerepel, mert ezt méri rá majd a gépállat a `points` paraméterre $\pm$.<br>
...
Érdemes még megfigyelni, hogy az Egyéb pártoknak a legszélesebb a konfidencia-intervalluma. Ez ugye azért van, mert mint láttuk a fejezet elején ide csak $n=13$ kitöltő tartozik. Így az $SH=\frac{s}{\sqrt{n}}$ összefüggés miatt érthető, hogy nagyon bizonytalanok vagyunk ennél a csoportnál az átlagos netezési idő becslésében. Ráadásul ez egy durva alábecslés is, amit itt a konfidencia-intervallum hosszára látunk! Hiszen a kis elemszám miatt a $k$ megbízhatósági szorzót itt $13-1$ szabadságfokú t-eloszlásból kellene számítani standard normális helyett, ahogy a `combine_effects` függvény csinálja. Persze, ez sem lenne teljesen pontos, mivel ahogy az alábbi hisztogram is szemlélteti, a napi netezési idők nem normális, hanem jobbra elnyúló eloszlásúak.
...
Ha valaki szeretné végigszámolni és ábrázoln a konfidencia-intervallumokat t-eloszlással, megteheti. Ezt viszont "manuálisan" kell megcsinálni a `groupby`-al létrehozott data frame oszlopok bővítésén keresztül.
...
Láthatóan tágultak a $99\%$-os konfidencia-intervallumok, de az még mindig igaz, hogy az Ellenzék átlagos ideje a sokaságban egyértelműen magasabb, mint a többi párt esetében tapasztalt még konfidencia-intervallumokkal együtt is. Bár a kis elemszámuknak köszönhetően az Egyéb pártra szavazók felső konfidencia-intervallum határa már majdnem beéri őket. :) Ez ugye várható volt, mert $n=13$ esetén azért sokat dob a $k$ megbízhatósági szorzó értékén az $n-1$ szabadságfokú t-eloszlás. :)